...
impl<'a> SolidBuilder<'a> {
    ...
    fn intern_edge(&mut self, id: u32) -> Result<EdgeId> {
        ...
        // An edge whose curve reaches neither of its vertices is not on that
        // curve. Two edges of the pilot's STEP carry a spline written 3–4 mm
        // from where the edge is: the same length and direction as the edge,
        // but displaced, so both vertices project onto one end of it, the
        // range collapses to a point, and the whole edge evaluates to one
        // point 3 mm off the faces it bounds — enough to cross a neighbouring
        // boundary segment, cost a cone its surface reading and leave the
        // rebuild 3.8 mm off. The Parasolid twin writes those edges as
        // straight lines between their vertices; the vertices are the one
        // thing the two descriptions of the edge agree on, so the edge is
        // their chord.
        let (curve, same_sense, range) = {
            ...
            let miss = |x: Vec3, p: Vec3| (x - p).length() > vertex_tol;
            let reaches_neither = miss(a, p0) && miss(a, p1) && miss(b, p0) && miss(b, p1);
            if reaches_neither && (p1 - p0).length() > vertex_tol {
                if std::env::var_os("CAD_STEP_RANGE_TRACE").is_some() {
                    eprintln!(
                        "[chord] #{id}: curve reaches neither vertex (ends miss by {:.4} / {:.4} mm, vertices {:.4} apart); edge is their chord",
                        (a - p0).length().min((a - p1).length()),
                        (b - p1).length().min((b - p0).length()),
                        (p1 - p0).length()
                    );
                }
                let cid = CurveId(self.solid.curves.len() as u32);
                self.solid.curves.push(Curve::Line { origin: p0, direction: p1 - p0 });
                (cid, true, Interval::new(0.0, 1.0))
            } else {
                (curve, same_sense, range)
            }
        };
