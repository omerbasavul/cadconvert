...
impl Curve {
    ...
    /// point.
    ///
    /// The conics wrap every turn. So does any other closed curve, over its own
    /// range: a closed spline or polyline is back where it started after one
    /// span, which is what a period is. Reporting only the conics left a
    /// surface swept along a closed spline looking non-periodic, and a face
    /// running right round it then read as one loop with another sitting
    /// beside it rather than as the band between them — the region came out
    /// wrong and the boundary crossed itself.
    pub fn period(&self) -> Option<f64> {
        match self {
            Curve::Circle { .. } | Curve::Ellipse { .. } => Some(TAU),
            Curve::Trimmed { base, .. } => base.period(),
            _ if self.is_closed() => {
                let span = self.natural_range().span();
...
/// Bring a parameter into the range the spline is defined on.
///
/// A B-spline says nothing outside its knots. Evaluating there runs the basis
/// functions past their support and the point leaves the curve — and it leaves
/// it fast, so an iteration allowed to wander outside will happily converge on
/// a parameter where the extrapolated curve passes through whatever it was
/// looking for. That is how a profile anchored five hundred metres up the
/// sweep axis, which the modeller wrote there quite deliberately, ends up
/// reporting a sweep distance of zero. A direction the curve closes in wraps,
/// which is exact; one it does not clamps, which is the nearest answer the
/// curve has.
fn nurbs_parameter(n: &NurbsCurve, t: f64) -> f64 {
    let Some((lo, hi)) = nurbs::domain(&n.knots, n.degree) else {
        return t;
    };
    if t >= lo && t <= hi {
        return t;
    ...
    let span = hi - lo;
    if n.closed && span > 0.0 {
        (t - ((t - lo) / span).floor() * span).clamp(lo, hi)
    } else {
        t.clamp(lo, hi)
    }
}
...
/// segment.
/// Fold a polyline's parameter into its range: round for a closed one, up
/// against the ends for an open one.
///
/// A closed polyline is a periodic curve and says so, so a parameter past its
/// last point belongs at the beginning again. Clamping there instead pinned
/// every such parameter on the final point, which is how an edge that runs
/// past the seam came back as a chain of one repeated position — and a bound
/// made of those reads as a degenerate point rather than as the loop it is.
fn polyline_parameter(points: &[Vec3], t: f64) -> f64 {
    let last = points.len() - 1;
    let span = last as f64;
    let closed = points.len() > 2 && (points[0] - points[last]).length_squared() < 1e-20;
    if closed && span > 0.0 {
        return t - (t / span).floor() * span;
    }
    t.clamp(0.0, span)
}
...
fn polyline_point(points: &[Vec3], t: f64) -> Vec3 {
    ...
    let last = points.len() - 1;
    let t = polyline_parameter(points, t);
...
fn polyline_derivative(points: &[Vec3], t: f64) -> Vec3 {
    ...
    let last = points.len() - 1;
    let i = (polyline_parameter(points, t).floor() as usize).min(last - 1);
    points[i + 1] - points[i]
}
...
mod tests {
    ...
    #[test]
    fn a_closed_polyline_wraps_instead_of_clamping() {
        // A square, closed. Its parameter runs 0..4 and then round again — a
        // curve that reports a period has to evaluate like one, or a parameter
        // past the seam pins on the last point and the edge comes back as a
        // chain of one repeated position.
        let square = Curve::Polyline {
            points: vec![
        ...
        assert_eq!(square.period(), Some(4.0));
        for t in [0.0, 0.5, 1.25, 3.75] {
            let here = square.point_at(t);
            let round = square.point_at(t + 4.0);
            assert!((here - round).length() < 1e-12, "at {t}: {here:?} vs {round:?}");
        }
        // And an open one still clamps, which is what it should do.
        let open = Curve::Polyline {
            points: vec![Vec3::ZERO, Vec3::new(1.0, 0.0, 0.0)],
        };
        assert_eq!(open.period(), None);
        assert_eq!(open.point_at(5.0), Vec3::new(1.0, 0.0, 0.0));
    }
    use super::*;
