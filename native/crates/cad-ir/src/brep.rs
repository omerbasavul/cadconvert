...
pub struct Solid {
    ...
    /// the data.
    pub tolerance: f64,
    /// Which surfaces were *measured* from the file's own geometry rather than
    /// interpolated from a face's boundary, indexed by [`SurfaceId`].
    ///
    /// A reader that cannot evaluate a surface — a Parasolid rolling-ball
    /// blend, whose exact form is a contact solve against both mates — stores
    /// a grid instead, and the grid is degree one in both directions so the
    /// tessellator meshes the face from its boundary. But two very different
    /// things arrive that way: a grid solved from the ball actually touching
    /// both surfaces, and a Coons patch interpolated from the boundary because
    /// nothing else could be had. The first is evidence about the interior;
    /// the second is a restatement of the boundary. Only the reader knows
    /// which, and without somewhere to say so the tessellator has to treat
    /// them alike.
    ///
    /// Empty means "nothing here is measured", which is the right answer for
    /// every reader that has no such fallback.
    pub measured: Vec<bool>,
}

impl Solid {
    /// Whether this surface was measured from the file's geometry. See
    /// [`Solid::measured`].
    pub fn is_measured(&self, surface: SurfaceId) -> bool {
        self.measured.get(surface.index()).copied().unwrap_or(false)
    }
}
...
impl Solid {
    ...
    pub fn geometric_bounds(&self) -> Aabb {
        ...
        // A writer is free to wrap the same circle in a trim, and one that
        // does hid every circle in the body from this consensus: the reference
        // fell back to the four clustered seam vertices of a turned ring, and
        // the tessellator then read the ring's own eight-millimetre circles as
        // runaways and collapsed them, losing two whole parts of the pilot
        // assembly. The wrapper is not the geometry — look through it.
        fn as_circle(c: &Curve) -> Option<(&Frame, f64)> {
            match c {
                Curve::Circle { frame, radius } if radius.is_finite() => Some((frame, *radius)),
                Curve::Trimmed { base, .. } => as_circle(base),
                _ => None,
            }
        }
        let mut radii: Vec<f64> = self
            .curves
            .iter()
            .filter_map(|c| as_circle(c).map(|(_, r)| r.abs()))
            .collect();
        if !radii.is_empty() {
        ...
        if self.vertices.len() < 8 {
            for c in &self.curves {
                ...
                {
                    let (a, c) = (semi_major.abs(), semi_minor.abs());
                    if a.is_finite() && c.is_finite() && frame.origin.is_finite() {
                        let r = ellipse_extent(frame, a, c);
                        b.add_point(frame.origin - r);
                        b.add_point(frame.origin + r);
                    }
                }
...
mod tests {
    ...
    #[test]
    fn geometric_bounds_sees_a_circle_through_a_trim() {
        use crate::math::Frame;
        // The same shaft, written with each circle wrapped in a trim — which
        // a writer is free to do and one in the pilot assembly does. Reading
        // only bare circles left the reference at the seam vertices, and the
        // tessellator then called the body's own circles runaways and
        // collapsed them, losing the part.
        let circle = |z: f64| Curve::Circle {
            frame: Frame::new(Vec3::new(0.0, 0.0, z), Vec3::Z, Vec3::X),
            radius: 10.0,
        };
        let s = Solid {
            curves: vec![
                Curve::Trimmed {
                    base: Box::new(circle(0.0)),
                    range: Interval::new(0.0, crate::math::TAU),
                },
                Curve::Trimmed {
                    base: Box::new(circle(25.0)),
                    range: Interval::new(0.0, crate::math::TAU),
                },
            ],
            ..Default::default()
...
/// Half-extent of a circle's axis-aligned bounding box.
///
/// A circle of radius r about a normal n reaches r·√(1 − n_k²) along axis k —
/// it spans the full radius across the axes its plane contains and nothing at
/// all along its own normal. Bounding it with the cube r in every direction is
/// the bound of the *sphere* through it, which for a body whose reference is
/// assembled from its circular edges reports an extent it does not have.
fn circle_extent(axis: Vec3, radius: f64) -> Vec3 {
    let n = axis.try_normalized().unwrap_or(Vec3::new(0.0, 0.0, 1.0));
    Vec3::new(
        radius * (1.0 - n.x * n.x).max(0.0).sqrt(),
        radius * (1.0 - n.y * n.y).max(0.0).sqrt(),
        radius * (1.0 - n.z * n.z).max(0.0).sqrt(),
    )
}

/// Half-extent of an ellipse's axis-aligned bounding box.
///
/// Parameterised as o + a·cos t·x̂ + c·sin t·ŷ, the extreme along axis k is
/// √((a·x̂_k)² + (c·ŷ_k)²) — the amplitude of the sum of two sinusoids a
/// quarter turn apart.
fn ellipse_extent(frame: &Frame, semi_major: f64, semi_minor: f64) -> Vec3 {
    let x = frame.ref_dir.try_normalized().unwrap_or(Vec3::new(1.0, 0.0, 0.0));
    let n = frame.axis.try_normalized().unwrap_or(Vec3::new(0.0, 0.0, 1.0));
    let y = n.cross(x);
    let reach = |xk: f64, yk: f64| ((semi_major * xk).powi(2) + (semi_minor * yk).powi(2)).sqrt();
    Vec3::new(
        reach(x.x, y.x),
        reach(x.y, y.y),
        reach(x.z, y.z),
