...
pub enum Compression {
    ...
    #[default]
    None,
    /// Quantise only the normals, to a byte a component, declaring
    /// `KHR_mesh_quantization`. Positions stay exactly what the tessellator
    /// computed.
    ///
    /// A byte a component resolves about 0.9°, well inside the angular
    /// tolerance the mesh was built to, and no vertex moves — so the solid
    /// stays exactly as watertight, and as free of slivers, as the plain
    /// output. Measured on the pilot: a quarter off the file for nothing that
    /// can be measured in the geometry.
    Normals,
    /// Quantise positions to 16-bit as well.
    ///
    /// The grid is each mesh's own bounding box, so on a 300 mm part it is
    /// 5 µm — an order of magnitude finer than the tessellation tolerance, and
    /// the pilot measures the same against OpenCASCADE either way. What it does
    /// cost is the mesh's finest slivers: about three thousand triangles across
    /// the pilot are narrower than the grid and collapse, which leaves the
    /// solid still closed but with some two hundred edges where three faces
    /// meet. Smallest, and not the one to hand on for further modelling.
    Quantized,
}
...
impl Options {
    ...
    /// The smallest output that changes no vertex: normals encoded a byte a
    /// component, positions exactly as computed.
    pub fn lean() -> Options {
        Options {
            compression: Compression::Normals,
            ..Options::default()
        }
    }
